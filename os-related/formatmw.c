...
/* This program was adjusted to build with z88dk but NEVER TESTED.

   I'm not sure which C was used originally
   it could have been the "Whitesmith's C"
...
void writebad ();
void dotest ();
void testit (unsigned char c, unsigned int pass);
int fmthead (unsigned int track, unsigned int head, unsigned char value);
void reset ();
void seek (int trk);
void select ();
void builds ();
void buildt (int ctrack);
void buildh (int head);
void werr (int errnum, int track, int head, int sector, int count);
void merr (int track, int head, int sector, int byte);
void verr (int errnum, int track, int head, int sector, int count);
void tverify (int track, int head, unsigned char data);
int issue ();
void attn ();
void prs (a);
void prn (a);
void prb (a);
void addto (int track, int head, int sector, int type);
...
unsigned char	image[IMSIZE] = 0;	/* Image for sector headers		*/
unsigned char	buffer[2048]  = 0;	/* Sector buffer			*/
...
//main (av,ac)
	...
	while (search->name != 0)
		{
		if (cmpstr (tolower(av[0]), search->name))
			{
			info = search;
			break;
			}
...
void verr (errnum, track, head, sector, count)
...
void merr (track, head, sector, byte)
...
void werr (errnum, track, head, sector, count)
	int errnum, track, head, sector, count;
	{
	prs ("Write: ");
	prs (errors[errnum]);
...
void prs (a)
	...
	for (;*a;a++)
		{
		if (bdos (11,0))			/* Console status ready */
			{
			c = bdos (6, 0xff);	/* Read the character */
			if (c == 3)
				exit (-2);
			if (c == 19)		/* Ctrl S ? */
				{
...
void addto (track, head, sector, type)
...
void report ()
	...
	qsort (bad, curbad, sizeof(struct bad), &order);
