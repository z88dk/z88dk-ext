...
// 
#define HAS__SCB(x) (((x) & 0xf0) >= TWO_X)
...
/****************************************************************************/
	...
	// a new condition introduced for CP/M 80
	if (ver <= 0x22) {
...
{                                               /*   current disk           */
        REG UWORD  k, all_blks;
        REG char   *all_vec, *all_end;
        LONG            maxall;
...
{
        REG UWORD  dimension;              /* Sets pointers to base of */
                                                /*   each array.  Returns   */
...
REG char   *va;                            /*   entry in table of vl   */
int     vl;                                     /*   entries at va          */
{                                               /* Return index into table  */
                                                /*   (> 0) if match found   */
        REG int    j, k, sync;             /*   else 0                 */
        BOOLEAN         found;
...
{
        int             d;
        REG UWORD  login;                  /* *UNSIGNED* login vector  */
...
{                                               /*   iobyte)                */
        REG UWORD  iobyte;                 /* Iobyte needs unsigned var*/
        REG int    j, k;
...
REG struct fcbtab *a;                      /*   names.  Returns -1 if  */
REG struct fcbhalf *b;                     /*   name of file a lexico- */
{                                               /*   graphically before that*/
        REG int    i, c_a, c_b;            /*   of b, 0 (FALSE) if     */
                                                /*   same, 1 if after       */
                                                /* Wildcards ('?') in name  */
...
{                                               /* Or add length of extent  */
        REG int    i, mb;                  /*   in Kbytes to *fcbma    */
...
{                                               /*   already encountered    */
        REG UWORD  i;                      /* Return index of matching */
                                                /*   entry or free slot     */
        fcbsa = fcbs;
...
{
        REG int    add, sizecols;          /* Layout variables         */
