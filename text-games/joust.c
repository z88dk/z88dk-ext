...
int defx[] = {0, 4, 5, 1, 3, 7, 1, 3,      0, 3, 4, 0, 6, 1, 0, 1,
          0, 0, 1, 3, 0, 3, 7, 5,      2, 3, 3, 1, 3, 7, 1, 3,
          4, 3, 7, 1, 0, 5, 1, 5,      0, 0, 6, 4, 3, 3, 1, 3};

int aimx[] = {0, 0, 0, 2, 4, 0,        4, 3, 0, 3, 3, 0,       5, 4, 1, 3, 7, 6,
...
int main (void) {
  ...
  do {
    ...
    puts (aimmsg[aimx[aim*8+aroll]]);
    if (defx[def*6+droll] < 4) {
      if (aimx[aim*8+aroll] > 3) {
        puts ("\nYou have won this joust.");
        r++;
        WAIT;
        puts (roundintro[r]);
        if (aimx[aim*8+aroll] == 5 && r < 3) {
          puts ("\nDue to your mighty victory in the last\nround your next opponent has conceded\nto you the win in this round.");
          r++;
          WAIT;
          puts (roundintro[r]);
        }
  ...
  } while (r < 4 && defx[def*6+droll] < 4);
  if (r < 4) {
    if (aimx[aim*8+aroll] < 4) {
      puts("\nToo bad, you lost. You leave the field\nof tournament a disgrace.");
    } else puts ("\nToo bad, you both lost. At least your\nhonor is intact.");
  }
  puts ("\nFarewell Sir Knight.");
  WAIT;
  return 0;
